...
def _assinatura_nome_completo(nome: str) -> tuple[str, str]:
    ...
    iniciais = "".join(_normalizar(t)[0] for t in tokens[:-1])
...
def _assinatura_abreviada(nome: str) -> tuple[str, str] | None:
    ...
    sobrenome = _normalizar(tokens[0])
    iniciais = "".join(_normalizar(t)[0] for t in tokens[1:] if len(t) == 1)
    if len(iniciais) != len(tokens) - 1:
        return None

    return sobrenome, iniciais
class ResolvedorIniciais:
